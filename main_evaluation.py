...
Examples:
  ...
  %(prog)s --target-text "Docker setup" --domain reliability --tech docker --output eval.json
    ...
    info_group.add_argument(
        "--list-domains", action="store_true", help="List available evaluation domains"
    )
    info_group.add_argument(
        "--list-compliance", action="store_true", help="List available compliance standards"
    )

    # Target prompt input
    target_group = parser.add_argument_group("Target Prompt Input")
    target_group.add_argument(
        "--target-file", metavar="FILE", help="File containing the prompt to evaluate"
    )
    target_group.add_argument(
        "--target-text", metavar="TEXT", help="Direct text of the prompt to evaluate"
    )

    # Evaluation configuration
    eval_group = parser.add_argument_group("Evaluation Configuration")
    eval_group.add_argument(
        "--domain", choices=[d.value for d in EvaluationDomain], help="Single evaluation domain"
    )
    eval_group.add_argument(
        "--chain",
        nargs="+",
    ...
    output_group.add_argument(
        "--output", "-o", metavar="FILE", help="Output file for evaluation prompt (JSON format)"
    )
    output_group.add_argument(
