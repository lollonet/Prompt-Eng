...
from src.types_advanced import (
    PromptConfigAdvanced, create_technology_name, create_task_type,
    PredefinedTemplates
)
from src.events import event_bus, setup_default_event_handlers, Event
from src.performance import performance_tracker
from src.result_types import Result, Success, Error
...
async def generate_example_prompt_modern(
    generator, 
...
async def setup_event_monitoring():
    ...
    event_bus.subscribe_sync(event_bus._handlers.get(list(event_bus._handlers.keys())[0], [None])[0].__class__ if event_bus._handlers else None, metrics_event_handler)
    
    logger.info("Event monitoring setup completed")


async def perform_health_checks(generator) -> Result[dict, str]:
    """
...
async def demonstrate_advanced_features():
    ...
    config_result = Result.Success(PromptConfigAdvanced(
        technologies=[create_technology_name("python")],
    ...
    final_result = (config_result
                   .and_then(lambda config: asyncio.create_task(generator.generate_prompt(config)))
                   .map(lambda prompt: f"Generated: {len(prompt)} characters"))
    
    if hasattr(final_result, 'unwrap'):  # Check if it's a Success
        logger.info(f"Advanced features demo: {final_result.unwrap()}")
