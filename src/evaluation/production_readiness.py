...
from .evaluation_types import (
    EvalContext, EvaluationResult, EvaluationMetrics, 
    Issue, Recommendation, RiskLevel, ComplianceStandard, TemplateType
)
...
class SecurityAnalyzer:
    ...
    def analyze(self, template: str, context: EvalContext) -> SecurityScore:
        ...
        overall_score = sum(result['score'] for result in security_checks.values()) / len(security_checks)
    ...
    def _check_network_security(self, template: str) -> Dict[str, Any]:
        ...
        sensitive_ports = ['22', '3306', '5432', '6379', '27017']  # SSH, MySQL, PostgreSQL, Redis, MongoDB
...
class PerformanceAnalyzer:
    ...
    def analyze(self, template: str, context: EvalContext) -> PerformanceScore:
        ...
        overall_score = sum(result['score'] for result in performance_checks.values()) / len(performance_checks)
    ...
    def _check_resource_limits(self, template: str) -> Dict[str, Any]:
        ...
        has_memory_limits = 'memory' in template.lower() and ('limit' in template.lower() or 'mem_limit' in template.lower())
    ...
    def _check_scaling_configs(self, template: str) -> Dict[str, Any]:
        ...
        has_scaling = any(term in template.lower() for term in ['replicas', 'scale', 'cluster_size'])
        if has_scaling:
            score = 0.8
        else:
            issues.append(Issue(
    ...
    def _check_monitoring_setup(self, template: str) -> Dict[str, Any]:
        ...
        has_health_checks = any(term in template.lower() for term in ['healthcheck', 'health', 'readiness', 'liveness'])
        if has_health_checks:
            score += 0.3
        
        # Check for metrics
        has_metrics = any(term in template.lower() for term in ['prometheus', 'metrics', 'monitoring'])
        if has_metrics:
            score += 0.2
...
class ReliabilityAnalyzer:
    ...
    def analyze(self, template: str, context: EvalContext) -> ReliabilityScore:
        ...
        overall_score = sum(result['score'] for result in reliability_checks.values()) / len(reliability_checks)
...
class ProductionReadinessEvaluator:
    ...
    def _assess_risks(self, metrics: Dict[str, EvaluationMetrics]) -> str:
        ...
        critical_issues = sum(len([i for i in m.issues if i.severity == RiskLevel.CRITICAL]) for m in metrics.values())
        high_issues = sum(len([i for i in m.issues if i.severity == RiskLevel.HIGH]) for m in metrics.values())
    ...
    def _assess_deployment_readiness(self, overall_score: float, metrics: Dict[str, EvaluationMetrics]) -> str:
    ...
    def _generate_summary(self, overall_score: float, metrics: Dict[str, EvaluationMetrics]) -> str:
        ...
        return (f"Production Readiness Score: {overall_score:.2f}/1.0. "
