...
from .types_advanced import TaskType, TechnologyName, TemplateName
...
class EventBus:
    ...
    def unsubscribe(
        self, event_type: EventType, handler: Union[EventHandler, SyncEventHandler]
    ) -> bool:
        """
...
async def metrics_collection_handler(event: Event) -> None:
    ...
    if event.event_type == EventType.PROMPT_GENERATION_COMPLETED:
        ...
        logger.debug(
            f"Metrics: prompt_generation_time={execution_time}ms, technologies={technologies_count}"
        )
...
def _create_function_payload(
    func: Callable, args: tuple, result_type: Optional[str] = None
) -> Dict[str, Any]:
...
async def _publish_async_events(
    config: EventPublishConfig, func: Callable, args: tuple, kwargs: dict
):
...
async def _example_event_usage():
    ...
    start_event = PromptGenerationStartedEvent.create(
        technologies=[TechnologyName("python")], task_type=TaskType("feature implementation")
    )
    await event_bus.publish(start_event)
