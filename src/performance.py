...
from typing import Any, AsyncContextManager, Callable, Dict, Optional, TypeVar, Union
...
class PerformanceTracker:
    ...
    def start_tracking(self, operation: str) -> str:
        ...
        self._metrics[operation] = PerformanceMetrics(
            operation_name=operation, execution_time=0.0, memory_usage_mb=current_memory
        )
        return operation
...
async def async_read_text_file(filepath: str):
    """
    ...
    try:
        ...
        if not path.exists():
            return Error(
                KnowledgeError(message=f"File not found: {filepath}", source="async_read_text_file")
            )

        if not path.is_file():
            return Error(
                KnowledgeError(
                    message=f"Path is not a file: {filepath}", source="async_read_text_file"
                )
            )
...
async def async_load_json_file(filepath: str):
    """
