...
class PerformanceViolation(Exception):
    ...
    def __str__(self) -> str:
        return (
            f"Performance violation: {self.threshold.name} "
...
class PerformanceGate:
    ...
    def __init__(
        ...
        custom_thresholds: Optional[Dict[str, PerformanceThreshold]] = None,
    ):
        """
    ...
    def _create_percentile_violation(
        self,
        threshold: PerformanceThreshold,
    ...
    def _check_individual_threshold(
        self, duration_ms: float, threshold: PerformanceThreshold, percentile: str
    ) -> None:
        """Check individual request against threshold."""
        if duration_ms > threshold.value:
            logger.warning(
                f"Individual request exceeded {percentile} threshold: {duration_ms:.2f}ms > {threshold.value}ms"
            )
    ...
    def check_memory_growth(self) -> None:
        ...
        if time_elapsed_hours > 0.1:  # Check after at least 6 minutes
            memory_growth = (
                (current_memory_mb - self._memory_baseline) / self._memory_baseline
            ) * 100
        ...
        if query_type == "simple":
            ...
            if duration_ms > threshold.value:
                violation = PerformanceViolation(
                    ...
                    context={"query_type": query_type, "duration_ms": duration_ms},
                )
    ...
    def get_violation_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "thresholds": {
                k: {"name": v.name, "value": v.value, "unit": v.unit, "description": v.description}
