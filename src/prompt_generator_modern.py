...
from .result_types import Result, Success, Error, PromptError, combine_results
from .types_advanced import (
    PromptConfigAdvanced, TechnologyName, TaskType, TemplateName,
    TemplateRenderer, KnowledgeSource, PredefinedTemplates,
    create_technology_name, create_task_type, create_template_name
)
from .knowledge_manager_async import AsyncKnowledgeManager, create_async_knowledge_manager
from .performance import (
    monitor_performance, performance_tracker, 
    async_performance_context, LazyEvaluator, lazy
)
from .events import (
    event_bus, EventType, 
    PromptGenerationStartedEvent, PromptGenerationCompletedEvent,
    publish_events
)
...
class ModernPromptGenerator(TemplateRenderer):
    ...
    def __init__(
        ...
        self._jinja_env = lazy(lambda: Environment(
        ...
        logger.info(f"Initialized ModernPromptGenerator with prompts_dir: {prompts_dir}")
    ...
    async def generate_prompt(self, config: PromptConfigAdvanced) -> Result[str, PromptError]:
        """
        ...
        try:
            ...
            logger.info(f"Generated prompt: {len(prompt)} characters in {execution_time:.3f}s")
            ...
            logger.error(f"Prompt generation failed: {error}")
            return Error(error)
    
    async def _collect_technology_data(
        self, 
        technologies: List[TechnologyName]
    ) -> Result[Dict[str, List[str]], PromptError]:
        """
        ...
        try:
            ...
            for result in tools_results:
                if result.is_error():
                    return Error(PromptError(
                        message=f"Failed to get tools: {result.error}",
                        code="KNOWLEDGE_ERROR"
                    ))
            ...
            return Success({
                'best_practices': unique_bp,
                'tools': unique_tools
            })
            
        except Exception as e:
            return Error(PromptError(
                message=f"Error collecting technology data: {str(e)}",
    ...
    async def _build_template_context(
        ...
        tech_data: Dict[str, List[str]]
    ) -> Result[Dict[str, Any], PromptError]:
        """
        ...
        try:
            ...
            bp_task = self._format_knowledge_items(
                ...
                self.knowledge_source.get_best_practice_details
            )
            tools_task = self._format_knowledge_items(
                tech_data['tools'],
                self.knowledge_source.get_tool_details
            )
            ...
            return Error(PromptError(
                message=f"Error building template context: {str(e)}",
                code="CONTEXT_ERROR"
            ))
    
    async def _format_knowledge_items(
        self,
        items: List[str],
        detail_getter: callable
    ) -> Result[List[str], PromptError]:
        """
        ...
        try:
            ...
            for item_name, result in zip(items, detail_results):
                if result.is_error():
                    ...
                    logger.warning(f"Could not get details for {item_name}: {result.error}")
            ...
            return Error(PromptError(
                message=f"Error formatting knowledge items: {str(e)}",
                code="FORMATTING_ERROR"
            ))
    
    async def render(self, template_name: TemplateName, context: Dict[str, Any]) -> Result[str, PromptError]:
        """
        ...
        try:
            ...
            async with self._cache_lock:
                if cache_key in self._template_cache:
                    ...
                    performance_tracker.record_cache_hit("template_render")
                else:
                    performance_tracker.record_cache_miss("template_render")
    ...
    async def health_check(self) -> Result[Dict[str, Any], PromptError]:
        """
        ...
        try:
            ...
            if hasattr(self.knowledge_source, 'health_check'):
                ...
                if knowledge_health.is_error():
                    return Error(PromptError(
                        message=f"Knowledge source health check failed: {knowledge_health.error}",
    ...
    def generate_prompt_legacy(
        ...
        try:
            ...
            result = loop.run_until_complete(
                self.modern_generator.generate_prompt(config)
            )
