...
from typing import (
    Protocol, Literal, Union, List, Dict, Any, Optional, 
    TypeVar, Generic, Callable, Awaitable, NewType
)
from typing_extensions import TypedDict, NotRequired
from abc import abstractmethod
from dataclasses import dataclass

from .result_types import Result, PromptError, KnowledgeError, Success, Error
...
class BestPracticeProvider(Protocol):
    ...
    @abstractmethod
    async def get_best_practices(self, technology: TechnologyName) -> Union[Success, Error]:
        """Get list of best practice names for a technology."""
        ...
    
    @abstractmethod  
    async def get_best_practice_details(self, name: BestPracticeName) -> Union[Success, Error]:
...
class ToolProvider(Protocol):
    ...
    @abstractmethod
    async def get_tools(self, technology: TechnologyName) -> Union[Success, Error]:
        """Get list of tool names for a technology."""
        ...
    
    @abstractmethod
    async def get_tool_details(self, name: ToolName) -> Union[Success, Error]:
...
class TemplateRenderer(Protocol):
    ...
    @abstractmethod
    async def render(self, template_name: TemplateName, context: Dict[str, Any]) -> Union[Success, Error]:
...
AsyncResult = Awaitable[Union[Success, Error]]
...
class PromptConfigAdvanced:
    ...
    def __post_init__(self):
        ...
        if len(self.code_requirements) < 10:
            raise ValueError(f"Code requirements must be detailed (≥10 chars), got {len(self.code_requirements)} chars")
...
def is_valid_technology_mapping(data: Any) -> bool:
    ...
    return (isinstance(data['best_practices'], list) and 
            isinstance(data['tools'], list) and
            all(isinstance(item, str) for item in data['best_practices']) and
            all(isinstance(item, str) for item in data['tools']))
...
def is_valid_tool_details(data: Any) -> bool:
    ...
    for field in optional_list_fields:
        if field in data and not (isinstance(data[field], list) and 
                                all(isinstance(item, str) for item in data[field])):
            return False
