...
class CircuitBreaker(ICircuitBreaker):
    ...
    async def _update_state(self) -> None:
        ...
        if self._state == CircuitState.OPEN:
            ...
            if (self._timeout_start_time and 
                current_time - self._timeout_start_time >= self._current_timeout):
            ...
            if (self._metrics.consecutive_failures >= self.config.failure_threshold or
                (self._metrics.total_requests >= 10 and 
                 self._metrics.failure_rate > 0.5)):
                await self._open_circuit()
