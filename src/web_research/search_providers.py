...
class SearchProviderFactory:
    ...
    @classmethod
    def create_provider(
        cls, config: SearchProviderConfig, circuit_breaker: Optional[CircuitBreaker] = None
    ) -> ISearchProvider:
...
class BaseSearchProvider(ISearchProvider):
    ...
    def __init__(
        self, config: SearchProviderConfig, circuit_breaker: Optional[CircuitBreaker] = None
    ):
        self.config = config
    ...
    async def _get_session(self) -> aiohttp.ClientSession:
        ...
        if self._session is None or self._session.closed:
            ...
            self._session = aiohttp.ClientSession(
                timeout=timeout, headers={"User-Agent": "Prompt-Engineering-CLI/1.0"}
            )
    ...
    async def search(self, query: str, max_results: int = 10) -> List[SearchResult]:
        ...
        if self._circuit_breaker:
            return await self._circuit_breaker.call(self._search_internal, query, max_results)
        else:
                        ...
                        results.append(
                            SearchResult(
                                ...
                                score=self._calculate_relevance_score(
                                    data.get("Heading", ""), data.get("Abstract", ""), query
                                ),
                        ...
                        if isinstance(topic, dict) and topic.get("Text"):
                            results.append(
                                SearchResult(
                                    ...
                                    score=self._calculate_relevance_score(
                                        topic.get("Text", ""), topic.get("Text", ""), query
                                    ),
                                    timestamp=datetime.now(),
    ...
    def _parse_bing_result(self, item: Dict[str, Any], query: str) -> Optional[SearchResult]:
        ...
        try:
            return SearchResult(
                ...
                score=self._calculate_relevance_score(
                    item.get("name", ""), item.get("snippet", ""), query
                ),
                timestamp=datetime.now(),
            ...
            return SearchResult(
                ...
                score=item.get(
                    ...
                    self._calculate_relevance_score(
                        item.get("title", ""), item.get("description", ""), query
                    ),
                ),
                timestamp=datetime.now(),
...
class MultiProviderSearchOrchestrator:
    ...
    async def search(
        self, query: str, max_results: int = 10, merge_results: bool = True
    ) -> List[SearchResult]:
        ...
        for provider in self.providers:
            try:
                ...
                self._logger.warning(f"Provider {provider.provider_name} failed: {e}")
                continue
