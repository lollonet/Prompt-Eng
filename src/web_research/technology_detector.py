...
class TechnologyDetector(ITechnologyDetector):
        ...
        if mapping_file.exists():
            try:
                ...
                for tech_name in mappings.keys():
                    ...
                    if any(keyword in tech_name.lower() for keyword in ['python', 'java', 'javascript', 'go', 'rust']):
                        self._knowledge.technology_categories[tech_name.lower()] = 'programming_language'
                    elif any(keyword in tech_name.lower() for keyword in ['react', 'vue', 'angular', 'express']):
                        self._knowledge.technology_categories[tech_name.lower()] = 'framework'
                    elif any(keyword in tech_name.lower() for keyword in ['postgresql', 'mysql', 'mongodb', 'redis']):
                        self._knowledge.technology_categories[tech_name.lower()] = 'database'
                    elif any(keyword in tech_name.lower() for keyword in ['docker', 'kubernetes', 'ansible']):
    ...
    async def _populate_default_knowledge(self) -> None:
        ...
        default_technologies = {
            ...
            'python': {'category': 'programming_language', 'popularity': 0.95, 'maturity': 'mature'},
            'javascript': {'category': 'programming_language', 'popularity': 0.98, 'maturity': 'mature'},
            'typescript': {'category': 'programming_language', 'popularity': 0.85, 'maturity': 'mature'},
            ...
            'elasticsearch': {'category': 'search_database', 'popularity': 0.75, 'maturity': 'mature'},
            ...
            'ansible': {'category': 'configuration_management', 'popularity': 0.80, 'maturity': 'mature'},
            'terraform': {'category': 'infrastructure_as_code', 'popularity': 0.85, 'maturity': 'mature'},
            ...
            # Specialized
            'patroni': {'category': 'database_clustering', 'popularity': 0.60, 'maturity': 'mature'},
        ...
        self._logger.info(f"Populated default knowledge with {len(default_technologies)} technologies")
    ...
    async def detect_unknown_technologies(self, technologies: List[str]) -> List[str]:
        ...
        for tech in technologies:
            ...
            best_match = max(similarity_results, key=lambda x: x.similarity_score) if similarity_results else None
    ...
    async def suggest_similar_technologies(self, technology: str) -> List[str]:
        ...
        suggestions = [
            result.technology for result in similarity_results
        ...
        if profile and profile.category != 'unknown':
            category_suggestions = [
                tech for tech, cat in self._knowledge.technology_categories.items()
    ...
    async def _find_similar_technologies(self, technology: str) -> List[SimilarityResult]:
        ...
        fuzzy_matches = difflib.get_close_matches(
            technology, 
            self._knowledge.known_technologies, 
            n=10, 
            cutoff=0.6
        )
        ...
        for result in results:
            if result.technology not in unique_results or result.similarity_score > unique_results[result.technology].similarity_score:
    ...
    async def learn_from_research(self, technology: str, research_data: Dict[str, Any]) -> None:
        ...
        if 'popularity_score' in research_data:
            self._knowledge.technology_popularity[tech_normalized] = research_data['popularity_score']
