...
class DockerTemplateEngine(ITemplateEngine):
    ...
    def supported_technologies(self) -> List[str]:
        return [
            ...
            "alertmanager", "victoria-metrics", "nginx", "redis",
            "postgresql", "mysql", "elasticsearch", "kibana"
        ]
    ...
    async def generate_template(self, context: TemplateContext) -> TemplateResult:
        ...
        if len(technologies) > 1:
            ...
            template_content = self._generate_dockerfile_template(technologies[0], context, docker_config)
    ...
    def _generate_compose_template(
        ...
        template_parts.extend([
            "", 
            self._generate_deployment_instructions(context)
        ])
