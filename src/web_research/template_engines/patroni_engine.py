...
class PatroniTemplateEngine(BaseTemplateEngine):
    ...
    def __init__(self):
        super().__init__(
            name="patroni",
            technologies=["patroni", "postgresql-cluster", "postgres-ha"]
        )
    ...
    def can_handle(self, context: TemplateContext) -> bool:
        ...
        return (
            context.specific_options.db_engine == "patroni" or
            "patroni" in context.technology.lower() or
            ("postgresql" in context.technology.lower() and 
             context.get_cluster_size() > 1)
        )
...
```"""
    ...
    def _generate_systemd_service(self, context: TemplateContext) -> str:
        ...
        commands = self._distro_commands.get(
            context.get_distro(), 
            self._distro_commands["rhel9"]
        )
...
```"""
    ...
    def get_quality_score(self, content: str, context: TemplateContext) -> float:
        ...
        patroni_indicators = [
            "scope:", "etcd:", "postgresql:", "bootstrap:",
            "authentication:", "replication:"
        ]
        
        present_indicators = sum(
            1 for indicator in patroni_indicators 
