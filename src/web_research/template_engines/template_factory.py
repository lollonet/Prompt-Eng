...
class TemplateEngineFactory:
    ...
    def select_best_engine(self, context: TemplateContext) -> Optional[ITemplateEngine]:
        ...
        if not compatible_engines:
            self._logger.warning(f"No compatible engines found for technology: {context.technology}")
            return None
    ...
    def _rank_engines(self, engines: List[ITemplateEngine], context: TemplateContext) -> List[ITemplateEngine]:
        """
        ...
        def engine_score(engine: ITemplateEngine) -> float:
            ...
            technology_matches = sum(
                1 for tech in engine.supported_technologies
                if tech in context.technology.lower()
            )
        ...
        self._logger.debug(
            f"Engine ranking for {context.technology}: "
            f"{[e.engine_name for e in ranked]}"
        )
