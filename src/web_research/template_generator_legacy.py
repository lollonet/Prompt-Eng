...
from .config import TemplateConfig, WebResearchConfig
...
class DynamicTemplateGenerator(IDynamicTemplateGenerator):
    ...
    async def generate_template(
        self, research: ResearchResult, specific_options: Optional["SpecificOptions"] = None
    ) -> str:
        """
    ...
    async def _try_engine_generation(
        self, research: ResearchResult, specific_options: "SpecificOptions"
    ...
    async def _generate_legacy_template(
        self, research: ResearchResult, specific_options: Optional["SpecificOptions"]
    ) -> str:
        """
...
```"""
                    ...
                    if relevance >= self._relevance_threshold:
                        code_examples.append(
                            CodeExample(
                                language=language,
                                code=code.strip(),
                                description=self._generate_code_description(
                                    code, search_result.title
                                ),
    ...
    async def _generate_section_content(
        ...
        if section_name == "task_description":
            ...
            return self._generate_implementation_steps(
                code_examples, best_practices, research.technology
            )
    ...
    def _generate_expected_output_section(
        self, code_examples: List[CodeExample], technology: str
    ) -> str:
        ...
        if self._current_specific_options:
            context_examples = self._generate_context_specific_examples(
                technology, self._current_specific_options
            )
            if context_examples:
...
{best_example.description}"""
    ...
    def _generate_implementation_steps(
        self, code_examples: List[CodeExample], best_practices: List[BestPractice], technology: str
    ) -> str:
        ...
        if any("security" in bp.category for bp in best_practices):
            steps.insert(
                3, "3. **Implement security measures** (authentication, validation, encryption)"
            )

        if any("performance" in bp.category for bp in best_practices):
            steps.insert(
                -2, f"{len(steps)}. **Optimize performance** with caching and efficient algorithms"
            )
    ...
    def _generate_quality_checklist(
        self, technology: str, best_practices: List[BestPractice]
    ) -> str:
    ...
    async def _assemble_template(
        self, research: ResearchResult, sections: List[TemplateSection], template_type: str
    ) -> str:
        ...
        header = structure["header"].format(
            technology=research.technology.title(), task_type=task_type
        )
        ...
        template_parts.append(
            f"Please implement step by step, explaining your choices for {research.technology} architecture and best practices."
        )
...
AWS infrastructure for {technology} in {options.region or 'us-west-2'}"""
    ...
    def _calculate_practice_confidence(
        self, practice_text: str, search_result: SearchResult, technology: str
    ) -> float:
    ...
    def _classify_source_type(self, url: str) -> str:
        ...
        community_domains = ["stackoverflow.com", "reddit.com", "dev.to", "medium.com"]

        tutorial_domains = ["tutorial", "guide", "how-to", "learn", "course"]
