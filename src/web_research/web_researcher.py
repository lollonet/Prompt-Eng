...
class WebResearcher(IWebResearcher):
        ...
        try:
            ...
            for query in queries:
                search_results = await self._search_orchestrator.search(
                    query, max_results=self.config.research.max_search_results_per_query
                )
    ...
    async def _filter_relevant_results(
        self, results: List[SearchResult], technology: str
    ) -> List[SearchResult]:
    ...
    async def _calculate_quality_score(
        self, results: List[SearchResult], best_practices: List[str], code_examples: List[str]
    ) -> float:
...
class ResearchOrchestrator(IResearchOrchestrator):
    ...
    async def _initialize_components(self):
        ...
        self._web_researcher = WebResearcher(
            self.config, self._search_orchestrator, self._circuit_breaker_manager
        )
    ...
    async def process_unknown_technologies(
        ...
        session = ResearchSession(
            session_id=session_id, technologies=technologies, user_context=user_context
        )
        ...
        try:
            ...
            unknown_technologies = await self._technology_detector.detect_unknown_technologies(
                technologies
            )
            ...
            self._logger.info(
                f"Session {session_id}: Researching {len(unknown_technologies)} technologies"
            )
            research_results = {}
            ...
            for i, technology in enumerate(unknown_technologies):
                ...
                if cached_research and await self._research_cache.is_research_fresh(technology):
                    ...
                    self._logger.info(
                        f"Researching {technology} ({i+1}/{len(unknown_technologies)})"
                    )
                    ...
                    quality = await self._research_validator.validate_research_result(
                        research_result
                    )
                    ...
                    if quality in [ResearchQuality.EXCELLENT, ResearchQuality.GOOD]:
                        ...
                        self._logger.warning(
                            f"Research quality for {technology} is {quality.value}"
                        )
                        # Could retry with different search strategy here
                        research_results[technology] = (
                            research_result  # Use anyway but mark as low quality
                        )
            ...
            for technology, research_result in research_results.items():
                try:
                    template = await self._template_generator.generate_template(
                        research_result, specific_options
                    )

                    # Validate template quality
                    template_quality = await self._template_generator.validate_template_quality(
                        template
                    )
                    ...
                    if template_quality >= self.config.research.min_quality_threshold:
                        ...
                        self._logger.warning(
                            f"Generated template quality for {technology} too low: {template_quality}"
                        )
                        results[technology] = await self._generate_fallback_template(
                            technology, research_result
                        )
            ...
            self._logger.info(
                f"Session {session_id}: Completed successfully with {len(results)} templates"
            )
            return results
...
Please implement step by step, explaining your choices for {technology} architecture and best practices."""
    ...
    async def _generate_fallback_template(
        self, technology: str, research_result: Optional[ResearchResult]
    ) -> str:
    ...
    def _infer_technology_category(self, research_result: ResearchResult) -> str:
        ...
        if any(keyword in technology for keyword in ["database", "postgres", "mysql", "mongo"]):
            ...
            keyword in technology for keyword in ["framework", "library", "react", "vue", "django"]
        ):
            return "framework"
