...
```
        ...
        # Python tools
        (tools_dir / "pytest.json").write_text(json.dumps({
            "name": "pytest",
        ...
        (tools_dir / "black.json").write_text(json.dumps({
            "name": "Black",
        ...
        # FastAPI tools
        (tools_dir / "pydantic.json").write_text(json.dumps({
            "name": "Pydantic",
...
- Docker configuration if needed
    ...
    def test_python_fastapi_postgresql_fullstack(self, realistic_knowledge_base):
        ...
        generator = PromptGenerator(
            prompts_dir=f"{base_path}/prompts",
    ...
    def test_complex_multi_technology_aggregation(self, realistic_knowledge_base):
        ...
        config = PromptConfig(
            technologies=["python", "fastapi", "react", "typescript", "postgresql", "docker", "kubernetes", "aws"],
        ...
        assert generation_time < 3.0, f"Complex generation took {generation_time:.2f}s, should be < 3s"
        
        # All technologies should be present
        all_techs = ["python", "fastapi", "react", "typescript", "postgresql", "docker", "kubernetes", "aws"]
        ...
        expected_practices = ["PEP8", "API Design", "Component Design", "Type Safety", "Database Design", "Containerization", "Orchestration", "Cloud Architecture"]
        practices_found = sum(1 for practice in expected_practices if practice.lower() in prompt.lower())
        assert practices_found >= 6, f"Only {practices_found}/8 expected practices found"
        
        # Should include tools from all technologies
        expected_tools = ["pytest", "pydantic", "jest", "tsc", "psycopg2", "dockerfile", "kubectl", "terraform"]
...
class TestKnowledgeAggregationLogic:
    ...
    def overlapping_knowledge_base(self, tmp_path):
        ...
        config_data = {
            ...
            "fastapi": {
                "best_practices": ["Clean Code", "API Design", "Documentation"],  # Overlap with python
    ...
    def test_knowledge_deduplication(self, overlapping_knowledge_base):
        ...
        expected_practices = {"Clean Code", "Testing", "PEP8", "MVC Pattern", "API Design", "Documentation"}
        actual_practices = set(best_practices)
        assert expected_practices.issubset(actual_practices), f"Missing practices: {expected_practices - actual_practices}"
        ...
        assert expected_tools.issubset(actual_tool_names), f"Missing tools: {expected_tools - actual_tool_names}"
    ...
    def test_knowledge_aggregation_order_consistency(self, overlapping_knowledge_base):
        ...
        assert practices_1 == practices_2 == practices_3, "Best practices should be consistent regardless of technology order"
        
        # Tools should also be consistent
        tools_1 = {tool["name"] if isinstance(tool, dict) else tool for tool in tech_data_1.get("tools", [])}
        tools_2 = {tool["name"] if isinstance(tool, dict) else tool for tool in tech_data_2.get("tools", [])}
        tools_3 = {tool["name"] if isinstance(tool, dict) else tool for tool in tech_data_3.get("tools", [])}
        
        assert tools_1 == tools_2 == tools_3, "Tools should be consistent regardless of technology order"
...
- Storybook stories (if applicable)
    ...
    def test_template_with_complex_data_structures(self, complex_template_setup):
        ...
        with patch.object(generator, '_collect_technology_data') as mock_collect:
            mock_collect.return_value = {
                ...
                "tools": [
                    {"name": "pytest", "description": "Testing framework", "usage": "pytest tests/"},
    ...
    def test_template_context_building_comprehensive(self, complex_template_setup):
        ...
        expected_keys = [
            "technologies", "task_type", "task_description", 
            "code_requirements", "best_practices", "tools"
        ]
...
class TestErrorHandlingAndResilience:
    ...
    def test_missing_knowledge_files_graceful_degradation(self, tmp_path):
        ...
        config_data = {
            "python": {
                "best_practices": ["PEP8", "Clean Code"],
                "tools": ["pytest", "black"]
            }
        }
...
class TestConcurrencyAndPerformance:
    ...
    def test_knowledge_manager_caching_under_load(self, tmp_path):
        ...
        for tool in ["pytest", "black", "jest", "eslint"]:
            (tools_dir / f"{tool}.json").write_text(json.dumps({
                "name": tool,
    ...
    def test_performance_with_realistic_datasets(self, tmp_path):
        ...
        technologies = ["python", "javascript", "typescript", "react", "vue", "angular", 
        ...
        for tech_list, description in test_scenarios:
            ...
            if len(tech_list) <= 5:
                assert generation_time < 1.0, f"{description} took {generation_time:.2f}s (should be < 1s)"
            elif len(tech_list) <= 10:
                assert generation_time < 2.0, f"{description} took {generation_time:.2f}s (should be < 2s)"
            else:
                assert generation_time < 3.0, f"{description} took {generation_time:.2f}s (should be < 3s)"
