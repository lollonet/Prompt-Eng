...
"""
    ...
    try:
        ...
        for task_id, task_result in results['task_results'].items():
            ...
            print(f"  {task_id}: {status} (Score: {task_result['score']:.3f}, Issues: {task_result['issues']})")
        
        # Detailed results for failed tasks
        failed_tasks = [
            (task_id, details) for task_id, details in results['detailed_results'].items()
...
def main():
    ...
    try:
        ...
        if 'summary' in benchmark_results:
            print(f"✅ HumanEval-DevOps Benchmark: {benchmark_results['summary']['pass_rate']:.1%} pass rate")
        else:
            print(f"❌ HumanEval-DevOps Benchmark: Failed")
        print(f"✅ Integration Test: {'PASS' if integration_results.get('production_ready') else 'CONDITIONAL'}")
