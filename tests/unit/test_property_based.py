...
from src.performance import LazyEvaluator, PerformanceMetrics, lazy
from src.performance_gates import PerformanceGate, PerformanceThreshold, PerformanceViolationType
from src.result_types import Error, Success, combine_results, safe_call
...
class TestPerformanceMetricsProperties:
    ...
    def test_performance_metrics_properties(self, operation_name, execution_time):
        ...
        metrics = PerformanceMetrics(
            operation_name=operation_name, execution_time=execution_time, memory_usage_mb=100.0
        )
    ...
    def test_metrics_to_dict_completeness(self, io_ops):
        ...
        metrics = PerformanceMetrics(
            operation_name="test_op", execution_time=2.5, memory_usage_mb=75.5, io_operations=io_ops
        )
...
class TestComplexSystemProperties:
    ...
    @given(
        st.lists(
            st.tuples(st.text(min_size=1), st.dictionaries(st.text(), st.integers())),
            min_size=1,
            max_size=10,
        )
    )
